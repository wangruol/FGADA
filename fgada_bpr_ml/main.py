...
# 生成器训练过程
def train_male_generator(male_like_emb,male_i_emb):
    ...
    targets = torch.full((outputs.size(0), outputs.size(1)), 1, device='cuda')
...
# 生成器训练过程
def train_female_generator(male_like_emb,male_i_emb):
...
class FairData(nn.Module):
    def __init__(self, user_num, item_num, factor_num,users_features,similarity,b,gcn_user_embs=None,gcn_item_embs=None):
        ...
        self.b=b
        # （num_embeddings，Embedding_dim）num_embeddings - 词嵌入字典大小，即一个字典里要有多少个词 embedding_dim - 每个词嵌入向量的大小
        self.embed_user = nn.Embedding(user_num, factor_num)
        self.embed_item = nn.Embedding(item_num, factor_num) 

        # .weight：得到的是一个parameter的变量，可以计算梯度的 normal正太分布为了使得网络中信息更好的流动，每一层输出的方差应该尽量相等。
...
#         self.zero_noise_item = torch.zeros(item_num, factor_num*2).cuda()
    ...
    # 定义生成器损失函数
    def generator_loss(self, generated_output, target):
        # reshaped_target = Reshape(tf.shape(generated_output))(target)
        reshaped_target = tf.reshape(target, shape=tf.shape(generated_output))
        # 使用 tf.subtract 计算差异
        diff_real = tf.subtract(generated_output, reshaped_target)
        return tf.reduce_mean(tf.square(tf.subtract(diff_real)))

    # 定义判别器损失函数
    def discriminator_loss(self, real_output, generated_output, target, real):
        # 使用 tf.reshape 将 embedding2 转换为和 embedding1 形状相同
        reshaped_target = tf.reshape(target, shape=tf.shape(real_output))
        reshaped_real = tf.reshape(real, shape=tf.shape(generated_output))
        # 使用 tf.subtract 计算差异
        diff_real = tf.subtract(real_output, reshaped_target)
        real_loss = tf.reduce_mean(tf.square(diff_real))
        diff_generated = tf.subtract(generated_output, reshaped_real)
        generated_loss = tf.reduce_mean(tf.square(diff_generated))
    ...
    def forward(self, u_batch,i_batch,j_batch):
        ...
        if male_like_emb.size(0) != 0:
            for epoch in tqdm(range(100)):
                ...
                gen_loss = train_male_generator(male_like_emb, female_i_emb)
                print('Epoch {}: G_Loss: {:.4f}, D_Loss: {:.4f}'.format(epoch, gen_loss, disc_loss))

            # 保存模型
            torch.save(male_generated.state_dict(), 'bpr_ml_male_generator.pth')
            torch.save(male_discriminator.state_dict(), 'bpr_ml_male_discriminator.pth')
            male_gan_emb = male_generated(male_like_emb).detach()
            # male_gan_emb = male_like_emb.detach()
        if female_like_emb.size(0) != 0:
            for epoch in tqdm(range(100)):
                ...
                gen_loss = train_female_generator(female_like_emb, male_i_emb)
                print('Epoch {}: G_Loss: {:.4f}, D_Loss: {:.4f}'.format(epoch, gen_loss, disc_loss))

            torch.save(female_generated.state_dict(), 'bpr_ml_female_generator.pth')
            torch.save(female_discriminator.state_dict(), 'bpr_ml_female_discriminator.pth')
            female_gan_emb = female_generated(female_like_emb).detach()
            # female_gan_emb = female_like_emb.detach()
        # 真假数据混合计算loss值
        # len_male_add = int(male_i_emb.size()[0] )
        # len_female_add = int(female_i_emb.size()[0] )
        ...
        l2_regulization = 0.01 * (u_emb ** 2 + j_emb ** 2).sum(dim=-1).mean() + 0.01 * (add_male_emb_i ** 2).sum(
            dim=-1).mean() + 0.01 * (add_female_emb_i ** 2).sum(
            dim=-1).mean()
...
parser.add_argument("--b", default=0.4, required=True, type=float, help="Options: 0.9,0.8,0.7,0.6,0.5,0.4,0.3,0.2,0.1")
...
for epoch in tqdm(range(150)):
    ...
    for user_batch, itemi_batch, itemj_batch, in train_loader:
        ...
        task_loss = get_loss
        loss_current.append(task_loss.item())
        task_optimizer.zero_grad()
        task_loss.backward()
        task_optimizer.step()
    ...
    train_loss_task = round(np.mean(loss_current), 4)
    ...
    if trained_end == 1:  # epoch==135:
        ...
        PATH_model_u_f1 = path_save_model_base + '/user_emb' + str(similarity) +str(b) + '3.npy'
        np.save(PATH_model_u_f1, user_e_f1)
        PATH_model_i_f1 = path_save_model_base + '/item_emb' + str(similarity)  +str(b) +'3.npy'
        np.save(PATH_model_i_f1, item_e_f1)
        print(PATH_model_i_f1)

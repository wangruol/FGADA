...
class GCN(nn.Module):
    def __init__(self, user_num, item_num, factor_num,users_features,adj_matrix,similarity,b):
        ...
        # .expand将单个维度扩大成更大维度，返回一个新的tensor
    ...
    def gcn_layer(self,users_embedding,items_embedding):
        ...
        # torch.sparse.mm（a，b）a是稀疏矩阵，b是稀疏矩阵或者密集矩阵，sparse.mm的作用和torch.mm一样，都是做矩阵乘法计算
    ...
    def forward(self, u_batch,i_batch,j_batch):    
        ...
        if male_like_emb.size(0) != 0:
            ...
            # 保存模型
            torch.save(male_generated.state_dict(), 'gccf_ml_male_generator.pth')
            torch.save(male_discriminator.state_dict(), 'gccf_ml_male_discriminator.pth')
        ...
        if female_like_emb.size(0) != 0:
            ...
            torch.save(female_generated.state_dict(), 'gccf_ml_female_generator.pth')
            torch.save(female_discriminator.state_dict(), 'gccf_ml_female_discriminator.pth')
        ...
        if male_like_emb.size(0) == 0:
            ...
            add_male_emb_i = torch.cat((male_i_emb[:-len_male_add], male_gan_emb.to(male_i_emb.device)[-male_i_emb:]),
                                       0)
        ...
        if female_like_emb.size(0) == 0:
            ...
            add_female_emb_i = torch.cat(
                (female_i_emb[:-len_female_add], female_gan_emb.to(female_i_emb.device)[-female_i_emb:]), 0)
...
for epoch in tqdm(range(87)):
    ...
    if trained_end == 1:
        PATH_model=path_save_model_base+'/best_model' + str(similarity) +str(b)+ '.pt'
        torch.save(model.state_dict(), PATH_model)
        
        PATH_model_u_f1=path_save_model_base+'/user_emb' +str(similarity) +str(b)+ '1.npy'
        np.save(PATH_model_u_f1,user_e_f1) 
        PATH_model_i_f1=path_save_model_base+'/item_emb' + str(similarity) +str(b)+ '1.npy'
